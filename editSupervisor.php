...
///////////////////////////////////////////////////////////////////////////////////////////				
		...
		$query="UPDATE Supervisor SET Supervisor_fname=?, Supervisor_lname=?, Dorm_ID=?, Supervisor_address=?, Supervisor_phone=? WHERE Supervisor_ID=?";
	
	
		//Prepare and execute query (use $_POST values from submitted form)
		$stmt=$mysqli->prepare($query);
		$stmt->execute([$_POST["spfirstname"], $_POST["splastname"], $_POST["dormid"], $_POST["spaddress"], $_POST["spphone"], $_POST["spid"]]);
		
		//Verify $stmt executed - create a SESSION message
		if($stmt){
			$_SESSION["message"]=$_POST["spfirstname"]." ".$_POST["splastname"]." is changed";
		} else{
			$_SESSION["message"]="Error changing".$_POST["spfirstname"]." ".$_POST["splastname"];
		}
...
///////////////////////////////////////////////////////////////////////////////////////////
		...
		if($stmt) {
			$_SESSION["message"] = $_POST["spfirstname"]." ".$_POST["splastname"]." has been changed";
			echo $_POST['spfirstname']." ".$_POST['splastname']." has been changed<br />";
		}
		else {
			$_SESSION["message"] = "Error! Could not add person";
			echo "Error! Could not change ".$_POST['spfirstname']." ".$_POST['splastname']."<br />";
		}
...
///////////////////////////////////////////////////////////////////////////////////////////
	  ...
	  //Prepare and execute a query to SELECT * using GET id in criterion - WHERE PersonID = ?
		$query="SELECT * from Supervisor WHERE Supervisor_ID=?";
		$stmt=$mysqli->prepare($query);
		$stmt->execute([$ID]);
		...
		if ($stmt)  {
			...
			echo "<h3>".$row["Supervisor_fname"]." ".$row["Supervisor_lname"]."'s Profile</h3>";
			...
			echo"<form action ='editSupervisor.php' method='post'>";
					//          Include <input> tags for each of the attributes in person:
					//                  First Name, Last Name, Birthdate, Birth City, Birth State, Region
					echo "<input type='hidden' name='spid' value='{$ID}'/>";
					echo "First Name: <input type=text name='spfirstname' value='{$row["Supervisor_fname"]}' />";
					echo "Last Name: <input type=text name='splastname' value='{$row["Supervisor_lname"]}' />";
					
					echo "Dorm ID: <input type=text name='dormid' value='{$row["Dorm_ID"]}' />";
					echo "Address: <input type=text name='spaddress' value='{$row["Supervisor_address"]}' />";
					echo "Phone number: <input type=text name='spphone' value='{$row["Supervisor_phone"]}' />";
